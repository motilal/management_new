...
class Gallery_model extends CI_Model {
    ...
    function __construct($options = null) {
        $this->response = array();
        $this->options = array(
            'script_url' => site_url('admin/galleries/upload'),
            'upload_dir' => dirname($this->get_server_var('SCRIPT_FILENAME')) . '/uploads/gallery/',
            'upload_url' => $this->get_full_url() . '/uploads/gallery/',
            'user_dirs' => false,
            'mkdir_mode' => 0755,
            'param_name' => 'files',
            ...
            'accept_file_types' => '/\.(gif|jpg|jpeg|tiff|png)$/i',
    ...
    function insertData($file_name) {
        if (!empty($file_name)) {
            $data = array('image' => $file_name, 'status' => '1');
    ...
    function deleteData($file_name) {
        if (!empty($file_name)) {
            ...
            if ($this->db->delete('galleries', $data)) {
                return true;
            }
        }
        return false;
